...
are all in backup1 and backup1 contains all SSTables in backup1 plus some new ones. Same holds for occupied space.

The figure of reclaimable space represent the number of bytes (or any human readable size) which would be freed
by deleting that particular backup. For example, from the above we see that by deleting backup-2, we would get
now free space. Why? Because all SSTables in backup-2 also belongs to backup-1. So we can not just physically remove it
...
physically remove only files which are part of that very backup and they are not resent anywhere
else.
