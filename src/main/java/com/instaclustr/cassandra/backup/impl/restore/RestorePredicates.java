...
class RestorePredicates {
    ...
    private static Optional<KeyspaceTable> getKeyspaceTableFromManifestPath(final String manifestLine) {
    ...
    static Predicate<Path> isSubsetTable(final RestorationEntities entities) {
...
//            differentiating the paths whenever they contain secondary indexes
    ...
    private static Predicate<String> getManifestFilesAllExceptSystem() {
        return manifestLine -> {
            final Optional<KeyspaceTable> ktOpt = getKeyspaceTableFromManifestPath(manifestLine);
    ...
    static Predicate<String> getManifestFilesForFullExistingRestore(boolean restoreSystemKeyspace) {
        ...
        if (restoreSystemKeyspace) {
            return m -> getKeyspaceTableFromManifestPath(m).isPresent();
        }
    ...
    static Predicate<String> getManifestFilesForSubsetExistingRestore(final RestorationEntities entities, boolean restoreSystemKeyspace) {
        ...
        return m -> {
            final Optional<KeyspaceTable> ktOpt = getKeyspaceTableFromManifestPath(m);

            return ktOpt.map(kt -> entities.contains(kt.keyspace, kt.table) || (restoreSystemKeyspace && kt.tableType == KeyspaceTable.TableType.SCHEMA))
                    .orElse(false);
        };
    }

    static Predicate<String> getManifestFilesForSubsetNewRestore(final RestorationEntities entities, boolean restoreSystemKeyspace) {
        ...
        return m -> {
            ...
            return ktOpt.map(kt -> kt.tableType == KeyspaceTable.TableType.SYSTEM_AUTH ||
                    ...
                    (restoreSystemKeyspace && kt.tableType == KeyspaceTable.TableType.SCHEMA) || entities.contains(kt.keyspace, kt.table))
                    .orElse(false);
        };
    }
}