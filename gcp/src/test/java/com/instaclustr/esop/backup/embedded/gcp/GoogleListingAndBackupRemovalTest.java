...
public class GoogleListingAndBackupRemovalTest extends BaseListingRemovalTest {
    ...
    protected String getStorageLocation() {
        return protocol() + AbstractBackupTest.BUCKET_NAME + "/cluster/datacenter1/node1";
    }

    protected String getStorageLocationForAnotherCluster() {
        return protocol() + AbstractBackupTest.BUCKET_NAME + "/cluster2/datacenter1/node1";
    }
    ...
    public void destroy() throws Exception {
        try {
            ...
            AbstractBackupTest.logger.error("Unable to remove Cassandra dir " + cassandraDir);
        }

        try {
            ListOperationRequest request = new ListOperationRequest();
            request.storageLocation = new StorageLocation(getStorageLocation());
            new GCPBucketService(storageFactory, request).delete(AbstractBackupTest.BUCKET_NAME);
        } catch (final Exception ex) {
